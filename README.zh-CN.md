...
在“设置”中填写服务 Base URL 和视觉模型名称；只有服务要求鉴权时才需要填写 API Key。“图片自动 OCR”和“图片自动生成描述”是两个独立开关，默认关闭。开启后，新捕获或导入的图片会在后台处理。OCR 固定发送提示词 `ocr this`；图片描述使用 ClipSave 内置的面向检索的完整提示词，并可在填写向量模型后用于语义搜索。
